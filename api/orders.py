...
def place_order(fyers_client, symbol: str, qty: int, side: str,
    ...
    try:
        order_data = {"symbol": symbol, "qty": qty, "type": order_type,
        ...
        if response.get("code") == 200:
            return {"order_id": response.get("id", ""), "status": "success",
                    "message": "Order placed successfully"}
        logger.error("Order error: %s", response)
...
def modify_order(fyers_client, order_id: str, qty: Optional[int] = None,
    ...
    try:
        ...
        response = fyers_client.update_order(data=data)
        if response.get("code") == 200:
...
def get_orderbook(fyers_client) -> list:
    try:
        ...
        logger.error("Orderbook exception: %s", e)
        return []
